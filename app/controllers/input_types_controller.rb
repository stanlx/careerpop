class InputTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @input_type.save
        ...
        format.json { render action: 'show', status: :created, location: @input_type }
      else
  ...
  def destroy
    @input_type.destroy
    respond_to do |format|
      format.html { redirect_to input_types_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def input_type_params
      params.require(:input_type).permit(:input_type_name)
    end
end
